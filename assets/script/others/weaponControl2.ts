...
import WeaponData from './weaponData2'
import EnemyParent from './enemyControl2'
import Weapon from './weapon2'
@ccclass
export default class WeaponControl extends cc.Component {
    ...
    onLoad() {
        ...
        for (let i = 1; i <= 3; i++) {
            cc.loader.loadRes(`others/prefab/weaponBtn${i}`, cc.Prefab, (err, prefab) => {
    ...
    addWeapon(index, event) {
        cc.loader.loadRes(`others/prefab/weapon${index}`, cc.Prefab, (err, prefab) => {
